...
try {
    ...
    [Khulnasoft.Azure.Common.Authentication.AzureSession]::ClientFactory.UserAgents.Add($AzureToolsUserAgentString)
} catch { }
...
if ($UploadArtifacts)
{
    ...
    & "$AzCopyPath" /Source:""$ArtifactStagingDirectory"" /Dest:$ArtifactsLocation /DestKey:$StorageAccountKey /S /Y /Z:""$env:LocalAppData\Khulnasoft\Azure\AzCopy\$ResourceGroupName""
...
$WebsiteName = (Get-AzureResource -ResourceGroupName $ResourceGroupName -ResourceType 'Khulnasoft.Web/sites').Name
...
{
    $storageAccountResource = Get-AzureResource -ResourceType 'Khulnasoft.ClassicStorage/storageAccounts' | Where-Object {$_.Name -eq $storageAccountName}
